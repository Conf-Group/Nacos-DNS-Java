 ...
 * <p>
 * // 8 byte start QDCOUNT: 2 byte ANCOUNT: 2 byte NSCOUNT: 2 byte ARCOUNT: 2 byte // 8 byte end
 *
...
public class Header {
    ...
    /**
     * ID field. The client will resolve the DNS reply message returned by the server and get the ID value to compare
     * with the ID value set by the request message. If the ID value is the same, it is considered to be the same DNS
     * session.
     */
    private int id;
    ...
    /**
     * Typically the value is 0 (standard query), the other values are 1 (reverse query) and 2 (server status
     * request),[3,15] reserved;
     */
    private short opcode;
    
    /**
     * Would its meaning be meaningful in the reply, indicating that the server answering the answer would be the
     * authoritative resolution server that would query the domain name;
     */
    private short aa;
    
    /**
     * Truncated - indicates that a message is longer than permitted, resulting in truncation
     */
    private short tc;
    
    /**
     * Represents Recursion Desired -- This bit is set by the request, and the same value is returned when the response
     * is made. If RD is set, recursive resolution is recommended for the domain name server. Recursive query support is
     * optional.
     */
    private short rd;
    
    /**
     * Recursion Available - This bit is set or cancelled in the reply to indicate whether the server supports recursive
     * queries.
     */
    private short ra;
    ...
    /**
     * The unsigned 16bit integer represents the number of problem records in the request segment of the message.
     */
    private int qdCount;
    
    /**
     * The unsigned 16bit integer represents the number of answer records in the message answer segment.
     */
    private int anCount;
    
    /**
     * The unsigned 16bit integer represents the number of authorized records in the authorization section of a
     * message.
     */
    private int nsCount;
    
    /**
     * The unsigned 16bit integer represents the number of additional records in the additional segment of the message.
     */
    private int arCount;
